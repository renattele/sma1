...
import com.team6.smartbudget.core.presentation.navigation.BottomSheetNavigator.Destination
 ...
 */
@Composable
fun rememberBottomSheetNavigator(skipPartiallyExpanded: Boolean = false): BottomSheetNavigator {
    var confirmValueChange by remember { mutableStateOf<(SheetValue) -> Boolean>({ true }) }
    ...
    return remember(sheetState) {
        BottomSheetNavigator(sheetState) { callback ->
            confirmValueChange = callback
        }
    }
}

/**
 * Navigator that drives a [SheetState] for use of [ModalBottomSheetLayout]s
 ...
 *
 * When the sheet is dismissed by the user, the [androidx.navigation.Navigator.state]'s [androidx.navigation.NavigatorState.backStack] will be popped.
 *
...
class BottomSheetNavigator(
    ...
    private val onConfirmValueChangeChanges: ((SheetValue) -> Boolean) -> Unit,
    ...
    /**
     * Get the back stack from the [androidx.navigation.Navigator.state]. In some cases, the [sheetInitializer] might be composed
    ...
    /**
     * Get the transitionsInProgress from the [androidx.navigation.Navigator.state]. In some cases, the [sheetInitializer] might be
            ...
            backStack
                .transform { backStackEntries ->
                    ...
                    if (entry != null) {
                        ...
                        onConfirmValueChangeChanges(destination.confirmValueChange)
                    }
                    value = entry
                }
        }
    ...
    /**
     * [androidx.navigation.NavDestination] specific to [BottomSheetNavigator]
     */
